...
const SelectButtonComponent = ({setValue , value , label , }) => {
  return <button  onClick={() => setCurrentCategory("odi")}
  style={{
    cursor: "pointer",
    color: currentCategory === "odi" ? "white" : "black",
    backgroundColor: currentCategory === "odi" ? "#0F19AF" : "white",
  }}
  className=" w-[80px] h-[40px] flex justify-center items-center rounded border small-text"></button>
}
...
const Womenrankingpage = () => {
  ...
  const testBestmanBody = testBestman?.map((item, index) => [
    <span>#{index + 1}</span>,
    <span
      style={{
        display: "flex",
