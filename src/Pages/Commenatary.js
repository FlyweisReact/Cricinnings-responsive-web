import qualifiedfinal from "../Assets/Homepage/qualifiedfinal.svg";
import topnews from "../Assets/Homepage/topnews.svg";
import videoframe from "../Assets/Homepage/videoframe.svg";
import { useParams } from "react-router-dom";
import Commentarynavbar from "../Components/Commentarynavbar";
import { useEffect, useState } from "react";
import axios from "axios";
import { baseUrl } from "../Components/Integration/ApiIntegration";
const Commenatary = () => {
  const params = useParams();
  const { matchId, commentary } = params;
  const [matchData, setMatchData] = useState([]);

  console.log(matchId);

  const getMatchDetails = async () => {
    axios.get(baseUrl + "user/getMatchById/" + matchId).then((res) => {
      setMatchData(res?.data?.data);
    })
  };

  useEffect(() => {
    getMatchDetails();
  }, []);

  return (
    <div className="">
      <div className="bg-[white] pl-2 pt-2">
        <Commentarynavbar />
        <div className="text-xl text-slate-500">Player Of The Match</div>
        <div className="text-xl">Suresh Raina</div>
      </div>
      <div className="bg-white pb-5  ">
        <div className="flex justify-center pt-2 gap-5">
          <div>
            <div className="">
              <div className="font-semibold">Match Info</div>
            </div>

            <div className="w-[680px]  mt-2 bg-white rounded-lg  shadow-lg flex justify-center  flex-wrap gap-5 pt-5 pb-5">
              <div>
                <span className="text-sm ml-4">India</span>
                <img src={qualifiedfinal} alt="" />
              </div>
              <div className="ml-4 mr-4">
                <div className="text-xl font-semibold">
                  Qualified For The FInals !!.... India Beats Newzland With 120
                  Runs
                </div>
                <div className="flex flex-col gap-2 mt-4">
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                </div>
              </div>
              <div className="ml-4">
                <div className="text-xl font-semibold">
                  Mohammad Rizwan | Multan captain (translated from Urdu):
                </div>
                <div className="flex flex-col gap-2 mt-4">
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                </div>
              </div>
              <div className="ml-4">
                <div className="text-xl font-semibold">
                  Usama Mir | Player of the Match (translated from Urdu):
                </div>
                <div className="flex flex-col gap-2 mt-4">
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                </div>
              </div>
              <div className="ml-4">
                <div className="text-xl font-semibold">
                  00:18 Local Time, 19:18 GMT, 00:48 IST: 
                </div>
                <div className="flex flex-col gap-2 mt-4">
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                  <div className="text-sm">
                    Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                    vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna
                    odio porta vel sed mi sagittis fermentum odio.
                  </div>
                </div>
              </div>

              <div className="flex gap-2 ml-4 w-[650px]">
                <span className="font-semibold">18.3</span>
                <div className="text-sm">
                  Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                  vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna odio
                  porta vel sed mi sagittis fermentum odio. velit metus rhoncus
                  enim dolor orci quis
                </div>
              </div>

              <div className="flex gap-2 ml-4 w-[650px]">
                <span className="font-semibold">18.3</span>
                <div className="text-sm">
                  Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                  vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna odio
                  porta vel sed mi sagittis fermentum odio. velit metus rhoncus
                  enim dolor orci quis
                </div>
              </div>

              <div className="flex gap-2 ml-4 w-[650px]">
                <span className="font-semibold">18.3</span>
                <div className="text-sm">
                  Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                  vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna odio
                  porta vel sed mi sagittis fermentum odio. velit metus rhoncus
                  enim dolor orci quis
                </div>
              </div>

              <div className="font-semibold ">
                Luke Wood [3.0-0-25-0] is back into the attack
              </div>
              <div className="flex justify-center">
                <div className="bg-[#0F19AF] w-[650px] h-[100px] flex justify-center items-center gap-3 rounded-lg">
                  <div className="text-white border-r pr-3 h-[50px] flex items-center">
                    18
                  </div>
                  <div className="text-white pr-3 border-r">
                    Runs Scored:
                    <br />
                     6 1 1 0 2 1 1
                  </div>
                  <div className="text-white pr-3 border-r">
                    Score after 18 overs
                    <br /> MI 139-3
                  </div>
                  <div className="text-white pr-3 border-r">
                    Iftikhar Ahmed 15(6)
                    <br />
                    Usman Khan 35(27)
                  </div>
                  <div className="text-white ">
                    Ravindra Jadeja
                    <br />
                    3-0-23-1
                  </div>
                </div>
              </div>

              <div className="flex gap-2 ml-4 w-[650px]">
                <span className="font-semibold">17.6</span>
                <div className="text-sm">
                  Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                  vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna odio
                  porta vel sed mi sagittis fermentum odio. velit metus rhoncus
                  enim dolor orci quis
                </div>
              </div>

              <div className="flex gap-2 ml-4 w-[650px]">
                <span className="font-semibold">17.4</span>
                <div className="text-sm">
                  Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                  vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna odio
                  porta vel sed mi sagittis fermentum odio. velit metus rhoncus
                  enim dolor orci quis
                </div>
              </div>

              <div className="flex gap-2 ml-4 w-[650px]">
                <span className="font-semibold">17.3</span>
                <div className="text-sm">
                  Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                  vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna odio
                  porta vel sed mi sagittis fermentum odio. velit metus rhoncus
                  enim dolor orci quis
                </div>
              </div>
              <div className="flex gap-2 ml-4 w-[650px]">
                <span className="font-semibold">17.6</span>
                <div className="text-sm">
                  Lorem ipsum dolor sit amet consectetur. Amet mus aliquam
                  vivamus tincidunt. Odio rhoncus pretium eu vivamus. Urna odio
                  porta vel sed mi sagittis fermentum odio. velit metus rhoncus
                  enim dolor orci quis
                </div>
              </div>
              <div className="font-semibold ">
                Luke Wood [3.0-0-25-0] is back into the attack
              </div>
              <div className="flex justify-center">
                <div className="bg-[#0F19AF] w-[650px] h-[100px] flex justify-center items-center gap-3 rounded-lg">
                  <div className="text-white border-r pr-3 h-[50px] flex items-center">
                    17
                  </div>
                  <div className="text-white pr-3 border-r">
                    Runs Scored:
                    <br />
                     6 1 1 0 2 1 1
                  </div>
                  <div className="text-white pr-3 border-r">
                    Score after 18 overs
                    <br /> MI 139-3
                  </div>
                  <div className="text-white pr-3 border-r">
                    Iftikhar Ahmed 15(6)
                    <br />
                    Usman Khan 35(27)
                  </div>
                  <div className="text-white ">
                    Ravindra Jadeja
                    <br />
                    3-0-23-1
                  </div>
                </div>
              </div>
              <div>
                <button className="text-slate-400 border w-[630px] h-[30px] rounded-lg">
                  Load More Commentary
                </button>
              </div>
            </div>
          </div>

          <div className="w-[250px] mt-10">
            <div className="bg-[#B3B3B3] text-white h-[550px]  flex justify-center items-center rounded-lg mt-2">
              RESPONSIVE AD’s
            </div>
            <div className="bg-[white]  rounded-lg shadow-2xl mt-2">
              <div className="text-sm p-3 font-semibold">FEATURE VIDEOS !!</div>
              <img src={videoframe} alt="" />
              <img src={videoframe} alt="" />
              <img src={videoframe} alt="" />
              <div className="flex justify-center pb-5">
                <button className="w-[100px] h-[30px] text-[12px] rounded flex justify-center items-center bg-[#0F19AF] text-white">
                  More Videos
                </button>
              </div>
            </div>

            <div className="bg-[#B3B3B3] text-white h-[550px]  flex justify-center items-center rounded-lg mt-2">
              RESPONSIVE AD’s
            </div>
            <div className="bg-[white] rounded-lg mt-2 shadow-2xl">
              <div className="p-1">
                <span className="font-semibold text-sm ml-4">TOP NEWS</span>
                <div className="flex mt-5">
                  <div className="w-[200px]">
                    <img src={topnews} alt="" />
                  </div>

                  <div>
                    <div className="text-[12px] font-bold">
                      Wankhade Stadium likely to host Ranji Final
                    </div>
                    <div className="text-slate-400 text-[10px]">
                      Mon,Mar03 2024
                    </div>
                  </div>
                </div>
                <div className="text-[12px] text-slate-400">
                  Lorem ipsum dolor sit amet consectetur. Elit eget mauris
                  egestas viverra urna sit. Tincidunt proin nulla dolor amet
                  purus adipiscing at ut. Nulla duis lorem venenatis mi dui
                  risus.
                </div>
                <div className="flex mt-5">
                  <div className="w-[200px]">
                    <img src={topnews} alt="" />
                  </div>

                  <div>
                    <div className="text-[12px] font-bold">
                      I found a 2007 study on effects of hand sanitizers
                    </div>
                    <div className="text-slate-400 text-[10px]">
                      Mon,Mar03 2024
                    </div>
                  </div>
                </div>
                <div className="text-[12px] text-slate-400">
                  Lorem ipsum dolor sit amet consectetur. Elit eget mauris
                  egestas viverra urna sit. Tincidunt proin nulla dolor amet
                  purus adipiscing at ut. Nulla duis lorem venenatis mi dui
                  risus.
                </div>
                <div className="flex mt-5">
                  <div className="w-[200px]">
                    <img src={topnews} alt="" />
                  </div>

                  <div>
                    <div className="text-[12px] font-bold">
                      The study was repeated with three brands of hand
                    </div>
                    <div className="text-slate-400 text-[10px]">
                      Mon,Mar03 2024
                    </div>
                  </div>
                </div>
                <div className="text-[12px] text-slate-400">
                  Lorem ipsum dolor sit amet consectetur. Elit eget mauris
                  egestas viverra urna sit. Tincidunt proin nulla dolor amet
                  purus adipiscing at ut. Nulla duis lorem venenatis mi dui
                  risus.
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Commenatary;
